...
let invalid_character pos = raise (Failure ("Invalid character found at " ^ (string_of_int pos)))
...
let rec consume_word word input pos =
  if pos < String.length input then
    ...
    match char with
    | 'a'..'z' | 'A'..'Z' | '0'..'9' -> consume_word (word ^ (String.make 1 char)) input (pos + 1)
    | _ -> pos, word
  else
    pos, word
;;
...
let rec consume_number number is_float input pos =
  let parse_result () =
    if is_float then
      FloatNumber (float_of_string number)
    else
      IntNumber (int_of_string number)
  in

  if pos < String.length input then
    let char = input.[pos] in
    match char with
    | '0'..'9' -> consume_number (number ^ (String.make 1 char)) is_float input (pos + 1)
    | '.' ->
        if is_float then
          invalid_character pos
        else
          consume_number (number ^ ".") true input (pos + 1)
    | _ -> pos, (parse_result ())
  else
    pos, (parse_result ())
;;
