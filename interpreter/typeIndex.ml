...
let union_create id params =
  match params with
  | param :: _ -> Value.Enum { variant = id; value = param }
...
let union_create_ext_function variants identifier =
  ...
  | Unit -> Value.Enum { variant = identifier; value = Value.Unit }
  | _ -> Value.ExternalFunction (union_create identifier)

let index_type (type_ : Type.t) (identifier : string) =
  match type_ with
  | Enum variants -> union_create_ext_function variants identifier
